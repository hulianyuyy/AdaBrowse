...
def evaluate(prefix="./", mode="dev", evaluate_dir=None, evaluate_prefix=None,
             ...
             triplet=False, evaluated_files=None):
    '''
    ...
    if python_evaluate:
        ret = wer_calculation(f"{evaluate_dir}/{evaluate_prefix}-{mode}.stm", f"{prefix}out.{output_file}", evaluated_files=evaluated_files)
        if triplet:
            wer_calculation(
