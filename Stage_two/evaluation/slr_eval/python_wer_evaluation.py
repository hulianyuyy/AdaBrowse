...
def load_groundtruth(fpath, evaluated_files=None):
    file_info = open(fpath, 'r', encoding='utf-8').readlines()
    gt_dict = dict()
    for line in file_info:
        name = line.split(" ")[0]
        if evaluated_files is not None and name in evaluated_files:
            info = line[:-1].split(" ")[5:]
            info = [*filter(lambda x: len(x), info)]
            gt_dict[name] = info
        elif evaluated_files == None:
            info = line[:-1].split(" ")[5:]
            info = [*filter(lambda x: len(x), info)]
            gt_dict[name] = info
    return gt_dict
...
def wer_calculation(gt_path, primary_pred, auxiliary_pred=None, evaluated_files=None):
    gt = load_groundtruth(gt_path, evaluated_files)
    ...
    if auxiliary_pred is not None:
        ...
        for fileid, sent in gt.items():
            ...
            results_list.append(sent_stat)
    #np.save("results_mid.npy", [data['wer_lstm'] / data['cnt'] for data in results_list])
