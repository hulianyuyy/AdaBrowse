...
class Optimizer(object):
    def __init__(self, model, optim_dict, num_epoch):
        ...
        if self.optim_dict["optimizer"] == 'SGD':
            ...
            raise ValueError()
        self.scheduler = self.define_lr_scheduler(self.optimizer, self.optim_dict['step'], num_epoch)

    def define_lr_scheduler(self, optimizer, milestones, num_epoch):
        if self.optim_dict["optimizer"] in ['SGD', 'Adam']:
            ...
            #lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, num_epoch)
            return lr_scheduler
        else:
            raise ValueError()
